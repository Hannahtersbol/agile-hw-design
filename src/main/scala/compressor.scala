...
class compressor(val sequencing: Int = 1) extends Module {
  ...
  val ch = Wire(Vec(sequencing, UInt(32.W)))
  val wi = Wire(Vec(sequencing, UInt(32.W)))
  val temp1 = Wire(Vec(sequencing, UInt(32.W)))
  val S0 = Wire(Vec(sequencing, UInt(32.W)))
  val maj = Wire(Vec(sequencing, UInt(32.W)))
  val temp2 = Wire(Vec(sequencing, UInt(32.W)))
  // Combinational hash round calculation
  for(i <- 0 until sequencing){
    S1(i) := bigSigma1(eWire(i))
    ch(i) := chF(eWire(i), fWire(i), gWire(i))
    wi(i) := blockRegs(loop_counter+i.asUInt)
    temp1(i) := (hWire(i) + S1(i) + ch(i) + K(loop_counter+i.asUInt) + wi(i))(31, 0)
    S0(i) := bigSigma0(aWire(i))
    maj(i) := majF(aWire(i), bWire(i), cWire(i))
    temp2(i) := (S0(i) + maj(i))(31, 0)

    aWire(i+1) := (temp1(i) + temp2(i))(31, 0)
    bWire(i+1) := aWire(i)
    cWire(i+1) := bWire(i)
    dWire(i+1) := cWire(i)
